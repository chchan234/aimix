    ...
    >
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="bg-pink-900/20 border border-pink-500 rounded-lg p-4 mt-6 mb-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white font-semibold">AI 닮은꼴 찾기</p>
                  <p className="text-gray-400 text-sm">얼굴 분석 + AI 매칭</p>
                </div>
      ...
      {step === 'upload' && !loading && (
        <div className="space-y-6">
          {/* Category Selection */}
          <div className="bg-gray-800 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-white mb-4">카테고리 선택</h3>
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            {imagePreview ? (
              ...
              <button
                onClick={() => fileInputRef.current?.click()}
                className="w-full p-8 border-2 border-dashed border-gray-600 rounded-lg hover:border-pink-500 transition-colors"
              >
      ...
      {loading && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">AI가 닮은꼴을 찾고 있습니다...</p>
        </div>
      )}
      ...
      {step === 'result' && result && (
        <div className="space-y-6">
                ...
                <div key={index} className="border border-gray-700 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-lg font-semibold text-white">{match.name}</span>
                    </div>
                    <span className={`text-xl font-bold ${getSimilarityColor(match.similarity)}`}>
                      {match.similarity}%
                    </span>
                  </div>
                  <p className="text-gray-300 mb-3">{match.reason}</p>
          ...
          {/* Face Analysis */}
          <div className="bg-gray-800 rounded-lg p-6">
            <h3 className="text-xl font-semibold text-white mb-4">얼굴 분석</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-gray-400 text-sm">얼굴형</p>
                <p className="text-white">{result.faceAnalysis.faceShape}</p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">눈</p>
                <p className="text-white">{result.faceAnalysis.eyeType}</p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">코</p>
                <p className="text-white">{result.faceAnalysis.noseType}</p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">입술</p>
                <p className="text-white">{result.faceAnalysis.lipType}</p>
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-gray-700">
              <p className="text-gray-400 text-sm">전체적인 인상</p>
              <p className="text-white">{result.faceAnalysis.overallImpression}</p>
            </div>
          </div>
