...
function buildUniversalPrompt(data: ResultImageData): string {
  const highlightsText = data.highlights
    .map((h, i) => `  ${i + 1}. ${h.label}: ${h.value}`)
    .join('\n');

  return `Create a professional Instagram-style result card (1080x1080px) with this exact structure:

HEADER (200px height):
- Background: Gradient ${data.colorGradient}
- Text: "${data.serviceName}" (white, 24px)
- Logo: "AI PORT" (white, bold, 32px, center)

MAIN CONTENT (680px height):
- Background: White
- Center alignment
- Main result: "${data.mainResult}" (64px, bold, dark gray)
- Sub result: "${data.subResult}" (28px, medium gray)
- 4 highlight boxes (each with light ${data.colorTheme} background):
${highlightsText}

FOOTER (200px height):
- Background: #f5f5f5
- Text: "aiports.org" (gray, 14px, center)
...
export async function generateResultImage(data: ResultImageData) {
  try {
    ...
    // Call API to generate image with credit deduction
    const response = await apiRequest<{ imageUrl: string }>('/api/image/generate-result-card', {
      prompt,
    ...
    link.download = `aiports-${data.serviceType}-result.png`;
    link.target = '_blank';
